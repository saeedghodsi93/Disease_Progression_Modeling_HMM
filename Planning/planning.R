...
planning.func <- function(run.algorithm, debugging.mode, latex.table) {
  ...
  if (run.algorithm == TRUE) {
    ...
    between.visit.time.params <- c(8,3)
    ...
    # planning parameters
    delta.tau.planning <- 5
    beta.z.planning <-c (2, 9, 25)
    beta.u.planning <-c (1, 3, 5)
    ...
    for (N.train.itr in 1:length(N.train.vals)) {
      ...
      ###############
      ################################ replace the input probabilities with hat (keep lambda and R)
      ################
      ret.expected.disutility <- planning.expected.disutility.func(N.test, I, J, L, H.test, a.test, z.obs.test, prob.posterior.lastobs, prob.posterior.history, lambda, R, Q, delta.tau.planning, beta.z.planning, beta.u.planning, between.visit.time.params)
      ...
      actual.disutility <- ret.expected.disutility$actual.disutility
