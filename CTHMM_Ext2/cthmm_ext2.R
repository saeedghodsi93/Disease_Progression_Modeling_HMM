...
# generate data, run the em algorithm and analyze the results for the CTHMM Ext2 model
cthmm.ext2.func <- function(run.algorithm, debugging.mode, latex.table) {
  ...
  if (run.algorithm == TRUE) {
    ...
    ret.set.model.parameters <- cthmm.ext2.set.model.parameters.func(latex.table)
    ...
    mu.hat.vals <- array(numeric(), dim=c(length(N.vals), counter.em.max+1, I,L))
    ...
    for (N.itr in 1:length(N.vals)) {
      ...
      ret.data.generation <- cthmm.ext2.data.generation(I, J, L, pi, lambda, R, Q, mu, eta, eta.prime, N, H.params, between.visit.time.params, accurate.examination.params)
      ...
      ret.EM.algorithm <- cthmm.ext2.EM.algorithm.func(N, I, J, L, H, a, O, tau.obs, z.acc, y.obs, u.obs, pi.init, Q.init, mu.init, eta.init, eta.prime.init, counter.em.max, debugging.mode)
    ...
    cthmm_ext2_store_raw(I, J, L, N.vals, H, a, O, tau.true, tau.obs, z.true, z.obs, z.acc, y.obs, u.obs)
    
    # store the results in file (for all the values of N)
    cthmm_ext2_store_results(pi, Q, mu, eta, eta.prime, pi.hat.vals, Q.hat.vals, mu.hat.vals, eta.hat.vals, eta.prime.hat.vals)
  
  } else {
    
    # load the raw data from file (only for the largest N)
    raw.cthmm.ext2 <- cthmm_ext2_load_raw()
    I <- round(raw.cthmm.ext2$I)
    J <- round(raw.cthmm.ext2$J)
    L <- round(raw.cthmm.ext2$L)
    N.vals <- raw.cthmm.ext2$N_vals
    H <- raw.cthmm.ext2$H
    a <- raw.cthmm.ext2$a
    O <- raw.cthmm.ext2$O
    tau.true <- raw.cthmm.ext2$tau_true
    tau.obs <- raw.cthmm.ext2$tau_obs
    z.true <- raw.cthmm.ext2$z_true
    z.obs <- raw.cthmm.ext2$z_obs
    z.acc <- raw.cthmm.ext2$z_acc
    y.obs <- raw.cthmm.ext2$y_obs
    u.obs <- raw.cthmm.ext2$u_obs

    # load the results from file (for all the values of N)
    res.cthmm.ext2 <- cthmm_ext2_load_results()
    pi <- res.cthmm.ext2$pi
    Q <- res.cthmm.ext2$Q
    mu <- res.cthmm.ext2$mu
    eta <- res.cthmm.ext2$eta
    eta.prime <- res.cthmm.ext2$eta_prime
    pi.hat.vals <- res.cthmm.ext2$pi_hat_vals
    Q.hat.vals <- res.cthmm.ext2$Q_hat_vals
    mu.hat.vals <- res.cthmm.ext2$mu_hat_vals
    eta.hat.vals <- res.cthmm.ext2$eta_hat_vals
    eta.prime.hat.vals <- res.cthmm.ext2$eta_prime_hat_vals
    
  }
  
  # use Python for visualization of the data
  cthmm_ext2_visualize_data(H, a, O, tau.true, tau.obs, z.true, z.obs, z.acc, y.obs, u.obs)
  
  # use Python for visualization of the results
  cthmm_ext2_visualize_results(I, J, L, N.vals, pi, Q, mu, eta, eta.prime, pi.hat.vals, Q.hat.vals, mu.hat.vals, eta.hat.vals, eta.prime.hat.vals)
